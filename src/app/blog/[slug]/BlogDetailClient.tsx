...
const CommentForm = ({ postId, onCommentSubmitted }: { postId: string, onCommentSubmitted: () => void }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await axios.post(`${API_URL}/comments/${postId}`, { content }, {
            ...
            onCommentSubmitted(); 
             ...
             const message = error.response?.data?.message || "خطا در ارسال نظر. لطفاً وارد شوید.";
             toast.error(message);
        } finally {
            setSubmitting(false);
        }
    ...
    if (!isAuthenticated) {
        return (
            <div className="bg-slate-800/50 p-6 rounded-xl text-center border border-slate-700 mb-8">
                <p className="text-gray-400 mb-4">برای ثبت نظر، لطفاً وارد حساب کاربری خود شوید.</p>
                <Link href="/auth/login" className="inline-block bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-lg transition">
                    ورود به حساب
                </Link>
            </div>
        );
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4 bg-slate-800/30 p-6 rounded-xl border border-white/5 mb-8">
            <h3 className="text-lg font-bold text-white mb-2">دیدگاه خود را بنویسید</h3>
            <textarea
                value={content}
    ...
    return (
        <div className="mt-16 pt-10 border-t border-white/10">
            <h2 className="mb-8 text-2xl font-bold text-white flex items-center gap-3">
                ...
                نظرات کاربران <span className="text-sm font-normal text-gray-500">({comments.length})</span>
            </h2>
            ...
            <div className="space-y-8">
                {comments.map((comment) => (
                    <div key={comment._id} className="flex gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            ...
                            {comment.adminReplyContent && (
                                <div className="flex gap-4 mr-8 mt-2">
                                    ...
                                    <div className="bg-green-900/20 rounded-2xl rounded-tr-none p-4 border border-green-500/20 flex-grow">
                                        <p className="text-xs font-bold text-green-400 mb-1">پاسخ انجمن:</p>
                                        <p className="text-gray-300 text-sm">{comment.adminReplyContent}</p>
                                    </div>
                                </div>
                            )}
                        </div>
        ...
        <div className="min-h-screen px-4 pt-24 pb-20 container mx-auto max-w-3xl text-white">
            <div className="flex items-center justify-between mb-8">
                <Link href="/blog" className="flex items-center gap-2 text-gray-400 hover:text-white w-fit group">
                    <ArrowLeft className="h-4 w-4 transition-transform group-hover:-translate-x-1" /> بازگشت به اخبار
                </Link>
            ...
            <article className="bg-slate-900/50 border border-white/10 rounded-2xl p-8 backdrop-blur-sm">
                ...
                <div className="flex items-center gap-6 text-sm text-gray-400 border-b border-white/10 pb-6 mb-6">
                    ...
                    <span className="flex items-center gap-2"><MessageSquare className="h-4 w-4 text-yellow-500"/> {comments.length} نظر</span>
                </div>
            ...
            {showDeleteModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-slate-900 border border-white/10 rounded-2xl p-6 max-w-sm w-full shadow-2xl transform scale-100 animate-in zoom-in-95 duration-200">
                        ...
                        <h3 className="text-xl font-bold text-white text-center mb-2">حذف این خبر؟</h3>
                        <p className="text-gray-400 text-center text-sm mb-6 leading-relaxed">
                            با حذف این خبر، تمام نظرات مرتبط با آن نیز حذف خواهند شد. آیا مطمئن هستید؟
                        </p>
