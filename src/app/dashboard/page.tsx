...
export default function DashboardPage() {
  ...
  const handleUserReply = async (ticketId: string, message: string) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `${API_URL}/contact/${ticketId}/reply`,
        { message },
        {
  ...
  const openCloseTicketModal = (ticketId: string) => {
  ...
  const confirmCloseTicket = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        `${API_URL}/contact/${ticketId}/close`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const saveEditedMessage = async (ticketId: string, messageId: string) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        `${API_URL}/contact/${ticketId}/messages/${messageId}`,
    ...
    try {
      ...
      setProfile((prev) => ({
        ...
        dateOfBirth: savedUser.dateOfBirth
          ? new Date(savedUser.dateOfBirth).toISOString().split("T")[0]
          : "",
      }));
      ...
      <main className="lg:w-3/4">
        ...
        {isTodayBirthday && (
          <div className="w-full bg-pink-900/40 border border-pink-500/50 p-4 rounded-xl text-white text-center mb-6 animate-pulse">
            <h3 className="font-bold text-xl text-pink-300">
              🎉 تولدت مبارک، {profile.name}! 🥳
            </h3>
            <p className="text-sm text-pink-200 mt-1">
              امیدواریم امروز بهترین روز سال برای شما باشد.
            </p>
          </div>
        )}
        ...
        {activeTab === "events" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {events.length === 0 ? (
              <div className="text-center py-16 bg-slate-900/50 rounded-2xl border border-dashed border-gray-700 text-gray-500/80">
                در هیچ رویدادی ثبت‌نام نکرده‌اید. <br />
                <Link
                  href="/events"
                  className="text-blue-400 hover:underline mt-2 inline-block"
                >
                  مشاهده لیست رویدادها
                </Link>
              </div>
            ) : (
              <div className="grid md:grid-cols-2 gap-4">
                {events.map((reg: any) => (
                  <div
                    key={reg._id}
                    className="bg-slate-900 border border-white/10 p-5 rounded-2xl hover:border-blue-500/50 transition group relative overflow-hidden"
                  >
                    <div
                    ...
                    <div className="text-sm text-gray-400 mb-4 space-y-2 bg-black/20 p-3 rounded-xl">
                      ...
                      {reg.receiptImage && (
                        ...
                        >
                          <FileText className="h-3 w-3" /> مشاهده رسید ارسالی
                        </a>
                      )}
                    </div>
        ...
        {activeTab === "messages" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-center border-b border-white/10 pb-4">
              ...
              >
                <MessageCircle className="h-4 w-4" /> تیکت جدید
              </Link>
            </div>
            {tickets.length === 0 ? (
                    ...
                    <div className="flex justify-between items-start mb-4 border-b border-white/5 pb-4">
                      <div>
                        ...
                        <span className="text-xs text-gray-500">
                          آخرین فعالیت: {toShamsiDate(ticket.updatedAt)}
                        </span>
                      </div>
                      {ticket.status === "OPEN" && (
                        <button
                          onClick={() => openCloseTicketModal(ticket._id)}
                          className="text-xs text-red-400 border border-red-400/20 px-2 py-1 rounded hover:bg-red-400/10 transition"
                        >
                          بستن گفتگو
                        </button>
                      )}
                          ...
                          {editingMessageId === msg._id ? (
                            <div className="w-full max-w-[80%] flex gap-2 items-end">
                              ...
                              <div className="flex flex-col gap-1">
                                <button
                                  onClick={() =>
                                    saveEditedMessage(ticket._id, msg._id)
                                  }
                                  className="bg-green-600 p-1.5 rounded-lg text-white hover:bg-green-500"
                                >
                                  <CheckCircle className="h-3 w-3" />
                                </button>
                                <button
                                  onClick={cancelEditing}
                                  className="bg-red-600 p-1.5 rounded-lg text-white hover:bg-red-500"
                                >
                                  <X className="h-3 w-3" />
                                </button>
                              </div>
                            ...
                            <div
                              className={`group relative max-w-[80%] p-3 rounded-xl text-sm ${
                                msg.sender === "USER"
                                ...
                                {msg.sender === "USER" &&
                                  ticket.status === "OPEN" && (
                                    <button
                                      ...
                                      className="mr-2 hover:text-white opacity-0 group-hover:opacity-100 transition-opacity"
                                      title="ویرایش"
                                    >
        ...
        {activeTab === "profile" && (
          <div className="max-w-xl animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
            ...
            >
              ...
              <button
                type="submit"
                className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition shadow-lg shadow-blue-600/20 mt-4"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-slate-900 border border-white/10 rounded-2xl p-6 max-w-sm w-full shadow-2xl transform scale-100 animate-in zoom-in-95 duration-200">
            ...
            <p className="text-gray-400 text-center text-sm mb-6 leading-relaxed">
              پس از بستن، شما و پشتیبانی دیگر قادر به ارسال پیام نخواهید بود.
              آیا مطمئن هستید؟
            </p>
