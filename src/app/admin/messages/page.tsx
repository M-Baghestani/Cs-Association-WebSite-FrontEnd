...
export default function AdminMessagesPage() {
  ...
  const confirmCloseTicket = async () => {
    ...
    const id = ticketToClose;
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        `${API_URL}/contact/${id}/close`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
        ...
        <div className="space-y-8">
          {messages.map((ticket) => (
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  <div
                    className={`flex h-10 w-10 items-center justify-center rounded-full text-white ${
                      ticket.user ? "bg-blue-600" : "bg-gray-600"
                    }`}
                  >
              ...
              <h4 className="mb-4 font-bold text-yellow-400 text-lg">
                ...
                {ticket.messages && ticket.messages.length > 0 ? (
                    ...
                    >
                      ...
                      >
                        ...
                        <span className="text-[10px] opacity-50 block mt-1">
                          {msg.sender === "ADMIN" ? "شما (ادمین)" : "کاربر"} -{" "}
                  ...
                  <>
                    <div className="flex justify-end">
                      <div className="bg-slate-800 p-3 rounded-xl text-sm text-gray-200">
                        {ticket.message}
                      </div>
                    </div>
                    {ticket.reply && (
                      <div className="flex justify-start mt-2">
                        <div className="bg-blue-900/50 p-3 rounded-xl text-sm text-white border border-blue-500/30">
                          {ticket.reply}
                        </div>
                      </div>
                    )}
              ...
              + {/* 🚨 MODAL: پنجره مودال اختصاصی برای بستن تیکت */}
              {showCloseModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                  <div className="bg-slate-900 border border-white/10 rounded-2xl p-6 max-w-sm w-full shadow-2xl transform scale-100 animate-in zoom-in-95 duration-200">
                    ...
                    <p className="text-gray-400 text-center text-sm mb-6 leading-relaxed">
                      پس از بستن، دیگر امکان ارسال پاسخ جدید برای این تیکت وجود
                      نخواهد داشت. آیا مطمئن هستید؟
                    </p>
